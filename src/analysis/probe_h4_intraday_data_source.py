...
SCRIPT_VERSION = "2026-08-26-v1-h4-intraday-source-feasibility"
SYMBOL = "SPY"
BASE_URL = "https://api.massive.com"
ET = ZoneInfo("America/New_York")
UTC = ZoneInfo("UTC")

# Ordinary full-session dates chosen to test historical reach across the study period.
SAMPLE_DATES = [
    "2021-01-04",
    "2022-06-15",
    "2023-10-02",
...
OUTPUT_PATH = Path("reports/data_quality/h4_intraday_source_feasibility.txt")
JSON_OUTPUT_PATH = Path("reports/data_quality/h4_intraday_source_feasibility.json")
...
def get_api_key() -> str:
    ...
    if not key:
        raise RuntimeError(
            "Set MASSIVE_API_KEY in the project environment before running this probe. "
            "POLYGON_API_KEY is also accepted as a legacy environment-variable name."
        )
    return key.strip()


def request_day(api_key: str, date_str: str) -> tuple[int, dict[str, Any]]:
    url = (
        f"{BASE_URL}/v2/aggs/ticker/{SYMBOL}/range/1/minute/"
        f"{date_str}/{date_str}"
    )
    params = {
        "adjusted": "false",
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            try:
                ...
                payload = {"status": "NON_JSON", "raw_prefix": resp.text[:500]}
...
def to_et(timestamp_ms: int) -> datetime:
    return datetime.fromtimestamp(timestamp_ms / 1000.0, tz=UTC).astimezone(ET)


def is_regular_session_minute(ts_et: datetime) -> bool:
    # Probe dates are intentionally ordinary full sessions.
    # Full production acquisition must later use the official session calendar
    # rather than hard-code 16:00 for early-close dates.
    tod = ts_et.timetz().replace(tzinfo=None)
    return dt_time(9, 30) <= tod < dt_time(16, 0)


def audit_day(date_str: str, status: int, payload: dict[str, Any]) -> DayAudit:
    if status != 200:
        msg = str(payload.get("error") or payload.get("message") or payload.get("status") or payload)
        return DayAudit(
            date=date_str,
            http_status=status,
            result_count=0,
    ...
    for row in rows:
        ...
        if None in (o, h, l, c):
            ...
            if h < max(o, l, c) or l > min(o, h, c) or min(o, h, l, c) <= 0:
                invalid_ohlc += 1
    ...
    rth_times = [to_et(ts) for ts in sorted(set(timestamps))]

    full_rth_coverage = (
        len(rth_times) == EXPECTED_FULL_RTH_MINUTES
        and duplicate_timestamps == 0
        and invalid_ohlc == 0
        and missing_volume == 0
        and nonpositive_volume == 0
        and bool(rth_times)
        and rth_times[0].strftime("%H:%M") == "09:30"
        and rth_times[-1].strftime("%H:%M") == "15:59"
    )
    ...
    return DayAudit(
        ...
        first_rth_et=rth_times[0].isoformat() if rth_times else None,
        last_rth_et=rth_times[-1].isoformat() if rth_times else None,
        access_ok=True,
...
def render_report(audits: list[DayAudit]) -> str:
    ...
    volume_ok = all(a.missing_volume == 0 and a.nonpositive_volume == 0 for a in audits)
    ohlc_ok = all(a.invalid_ohlc == 0 for a in audits)
    unique_ok = all(a.duplicate_timestamps == 0 for a in audits)

    vwap_available_everywhere = all(a.missing_vwap == 0 for a in audits)
    txn_available_everywhere = all(a.missing_transactions == 0 for a in audits)

    gate_pass = all_access and all_full and volume_ok and ohlc_ok and unique_ok

    lines = [
        f"RUNNING SCRIPT VERSION: {SCRIPT_VERSION}",
        "=" * 110,
        "H4 INTRADAY CONSOLIDATED-MINUTE SOURCE FEASIBILITY",
        "=" * 110,
        f"Symbol: {SYMBOL}",
    ...
    for a in audits:
        lines.extend([
            ...
            f"Historical access: {'PASS' if a.access_ok else 'FAIL'}",
    ...
    lines.extend([
        ...
        "SOURCE GATE SUMMARY",
        "=" * 110,
        f"Historical access across all sample years: {'PASS' if all_access else 'FAIL'}",
        ...
        f"Provider trade-based VWAP present on all probe RTH rows: {'YES' if vwap_available_everywhere else 'NO / REVIEW'}",
    ...
    if gate_pass:
        lines.extend([
            "",
            "H4_CONSOLIDATED_MINUTE_SOURCE_FEASIBILITY_GATE_PASSED",
            "",
            "Next authorized action:",
            "Build a full-history acquisition manifest for SPY 2021-01-01 through 2025-12-31,",
            "then audit the raw 1-minute history before deriving any H4 trigger outcomes.",
        ])
    else:
        lines.extend([
            "",
            "NO FULL H4 INTRADAY ACQUISITION IS AUTHORIZED BY THIS RUN.",
            "Resolve source/history/session completeness first.",
        ])
...
def main() -> None:
    ...
    for date_str in SAMPLE_DATES:
        print(f"Probing {SYMBOL} 1-minute history for {date_str} ...")
        status, payload = request_day(api_key, date_str)
    ...
    OUTPUT_PATH.write_text(render_report(audits), encoding="utf-8")
