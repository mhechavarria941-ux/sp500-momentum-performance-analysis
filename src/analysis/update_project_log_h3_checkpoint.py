...
PROJECT_LOG = ROOT / "docs" / "project_log.md"

SCRIPT_VERSION = "2026-08-24-v1-project-log-h3-checkpoint-update"

INSERT_TEXT = r"""## 3.46 H2 Sector-Relative Momentum Implementation, Validation, and Final Closeout
...
def main() -> None:
    ...
    if not PROJECT_LOG.exists():
        raise FileNotFoundError(PROJECT_LOG)

    original = PROJECT_LOG.read_text(encoding="utf-8")
    ...
    if not cut_positions:
        raise RuntimeError(
            "Could not find either the existing 3.46 checkpoint or "
            "the final '# Current Status' section in docs/project_log.md."
        )

    cut_at = min(cut_positions)
    prefix = original[:cut_at].rstrip()

    updated = prefix + "\n\n---\n\n" + INSERT_TEXT.strip() + "\n"

    # Atomic replacement: write temp in the same directory then replace.
    fd, temp_name = tempfile.mkstemp(
        prefix="project_log_",
        suffix=".tmp",
        dir=str(PROJECT_LOG.parent),
        text=True,
    )
    try:
        with os.fdopen(fd, "w", encoding="utf-8", newline="\n") as handle:
            handle.write(updated)
        Path(temp_name).replace(PROJECT_LOG)
    except Exception:
        Path(temp_name).unlink(missing_ok=True)
        raise

    print(f"Updated: {PROJECT_LOG}")
    print("Previous stale Current Status section replaced.")
    print("Canonical continuation now runs through section 3.55.")
    print("PROJECT_LOG_H3_CHECKPOINT_UPDATE_COMPLETE")
